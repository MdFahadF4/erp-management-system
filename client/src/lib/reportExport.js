import {
  ...
  getCompanyInfo,
  getCompanyLegalLine,
  getCompanyDisplayTitle
} from '../config/company.js';
import { t } from './reportI18n.js';

export { getCompanyInfo, getCompanyLegalLine, getCompanyDisplayTitle };
...
function collectReportTables(root, tableContainerId = 'report-table-container') {
  ...
  return [...scope.querySelectorAll('table')].map((table, idx) => {
    const titleEl = table.closest('div.border')?.querySelector(
      '.bg-slate-800, .bg-gray-800, .bg-violet-50, .bg-blue-50, .font-bold.p-3'
    );
...
async function exportReportRootAs(format, profile) {
  ...
  if (format === 'ppt') {
    try {
      ...
      slide1.addText(co.COMPANY_NAME, {
        x: 0.5,
        y: 0.8,
        w: 9,
        h: 0.6,
      ...
      if (summary.rows.length) {
        const slide = pptx.addSlide();
        slide.addText(t('report.exportSummarySection'), {
          x: 0.3,
          y: 0.2,
          w: 9.4,
          h: 0.4,
        ...
        slide.addTable(
          summary.rows.filter((r) => r.length),
          {
            x: 0.3,
            y: 0.7,
            w: 9.4,
