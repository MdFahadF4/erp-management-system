export const templates = {
  ...
  hr_factory: `
      ...
      <div id="hr-factory-panel-details" class="hidden space-y-4">
        ...
        <div id="hr-factory-export-toolbar" class="hr-factory-export-toolbar hidden print:hidden flex flex-wrap gap-2 p-3 bg-white border border-gray-200 rounded-lg shadow-sm">
          <button id="btn-hr-factory-print" type="button" class="flex-1 sm:flex-none bg-slate-800 hover:bg-slate-900 text-white font-bold px-3 py-2 rounded text-xs transition shadow-sm" data-i18n="common.print">Print</button>
          <button id="btn-hr-factory-pdf" type="button" class="flex-1 sm:flex-none bg-red-700 hover:bg-red-800 text-white font-bold px-3 py-2 rounded text-xs transition shadow-sm">PDF</button>
          <button id="btn-hr-factory-word" type="button" class="flex-1 sm:flex-none bg-blue-700 hover:bg-blue-800 text-white font-bold px-3 py-2 rounded text-xs transition shadow-sm">Word</button>
          <button id="btn-hr-factory-excel" type="button" class="flex-1 sm:flex-none bg-emerald-700 hover:bg-emerald-800 text-white font-bold px-3 py-2 rounded text-xs transition shadow-sm">Excel</button>
          <button id="btn-hr-factory-ppt" type="button" class="flex-1 sm:flex-none bg-orange-600 hover:bg-orange-700 text-white font-bold px-3 py-2 rounded text-xs transition shadow-sm">PPT</button>
        </div>

        <div id="hr-factory-report-results" class="erp-factory-report-results erp-report-results hidden bg-white p-3 md:p-5 rounded-xl shadow border border-gray-200 flex flex-col overflow-visible print:shadow-none print:border-none print:p-0">
          <div id="hr-factory-report-print-header" class="hidden mb-4 md:mb-6 border-b pb-4 print:block">
            <div class="erp-report-print-meta flex items-start justify-between gap-4 px-2">
              <div class="erp-report-print-center flex-1 text-center">
                <h1 class="text-lg md:text-2xl font-black text-gray-900 uppercase tracking-wide" id="hr-factory-report-company-name" data-company-name>Mehrin Trading Co.</h1>
          ...
          <div id="hr-factory-details-table" class="erp-report-panel order-1 bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden print:border-none"></div>
          <div id="hr-factory-details-summary" class="hidden grid grid-cols-1 mb-2 order-2 erp-report-summary-footer"></div>
          <div class="erp-print-page-footer hidden print:block order-3 text-center text-[10px] text-gray-500 pt-2">
            <span id="hr-factory-print-footer-datetime"></span>
          </div>
        </div>
      </div>
    </div>
        ...
        <div id="form-container" class="bg-white p-5 rounded-xl shadow border border-gray-200 xl:col-span-4 max-w-2xl mx-auto w-full max-h-none md:max-h-[85vh] overflow-y-auto pb-24 md:pb-5 transition-all duration-300">
          ...
          <form id="form-sup-txn-entry" class="space-y-4 text-xs">
            ...
            <div class="p-3 bg-gray-50 border rounded-lg space-y-3">
              ...
              <p class="text-[10px] text-gray-400 -mt-1" data-i18n="field.purchaseAmountHint">Choose Category (Purchase / Payment Paid / Previous Due). For opening balance, select Previous Due and enter the amount in Purchase.</p>
  ...
  customer_transactions: `
      ...
      <div class="grid grid-cols-1 xl:grid-cols-4 gap-6">
        <div id="form-container" class="bg-white p-5 rounded-xl shadow border border-gray-200 xl:col-span-4 max-w-2xl mx-auto w-full max-h-none md:max-h-[85vh] overflow-y-auto pb-24 md:pb-5 transition-all duration-300">
          <h3 id="cust-txn-form-title" class="text-sm font-bold text-gray-700 mb-4 uppercase tracking-wider" data-i18n="form.cust.logPayment">Log Customer Payment</h3>
          <div class="flex items-center gap-2 mb-3">
            <div class="flex flex-1 rounded-lg border border-gray-200 overflow-hidden text-[11px] font-bold min-w-0">
              <button type="button" id="cust-txn-mode-normal" class="flex-1 px-3 py-2 bg-blue-600 text-white transition" data-i18n="custTxn.modeNormal">Normal Sale / Payment</button>
              <button type="button" id="cust-txn-mode-refund" class="flex-1 px-3 py-2 bg-gray-100 text-gray-600 hover:bg-gray-200 transition" data-i18n="custTxn.modeRefund">Refund / Cancellation</button>
            </div>
          ...
          <form id="form-cust-txn-entry" class="space-y-4 text-xs">
            ...
            <div><label class="block font-bold text-gray-600 mb-1" data-i18n="field.soldAmountOptional">Sold Amount (optional)</label><input type="number" step="0.01" id="cust-txn-sell" value="0" class="w-full border rounded p-2 text-sm outline-none" placeholder="0.00" data-i18n-placeholder="placeholder.zeroPaymentOnly"></div>
            <p id="cust-txn-sold-hint" class="text-[10px] text-gray-400 -mt-2" data-i18n="field.soldAmountOptionalHint">Leave 0 when customer is only paying a previous due balance.</p>
      ...
      <div class="grid grid-cols-1 xl:grid-cols-4 gap-6">
        <div id="form-container" class="bg-white p-5 rounded-xl shadow border border-gray-200 xl:col-span-4 max-w-2xl mx-auto w-full max-h-none md:max-h-[85vh] overflow-y-auto pb-24 md:pb-5 transition-all duration-300">
          ...
          <form id="form-exp-txn-entry" class="space-y-4 text-xs">
            ...
            <div><label class="block font-bold text-gray-600 mb-1" data-i18n="field.categoryClassification">Category Classification</label><select id="exp-txn-category" required class="w-full border rounded p-2 bg-white text-sm outline-none focus:ring-2 focus:ring-red-500"><option value="Incurred" data-i18n="field.totalDepositIncurred">Incurred (Increases Due)</option><option value="Payment Paid" data-i18n="category.paymentDecreases">Payment Paid (Decreases Due)</option></select></div>
            <div id="exp-txn-due-info" class="hidden bg-red-50 border border-red-100 rounded-lg p-3 space-y-1.5">
              <div class="flex justify-between items-center gap-2">
                <span class="font-bold text-red-800 text-[11px] uppercase" data-i18n="field.currentAccountDue">Current Due / Balance</span>
                <span id="exp-txn-current-due" class="font-mono font-black text-red-700 text-sm">0.00</span>
              </div>
              <div class="flex justify-between items-center gap-2 border-t border-red-100 pt-1.5">
                <span class="font-bold text-gray-600 text-[11px] uppercase" data-i18n="field.remainingDueAfterTxn">Remaining Due After This Transaction</span>
                <span id="exp-txn-remaining-due" class="font-mono font-bold text-orange-700 text-sm">0.00</span>
              </div>
            </div>
            <div class="p-3 bg-gray-50 border rounded-lg space-y-3">
              ...
              <div><label class="block font-bold text-purple-700 mb-1" data-i18n="field.discountAllowed">Discount Allowed</label><input type="number" step="0.01" id="exp-txn-discount" value="0.00" class="w-full border rounded p-2 text-sm font-bold font-mono outline-none focus:ring-2 focus:ring-purple-500"></div>
      ...
      <div class="grid grid-cols-1 xl:grid-cols-4 gap-6">
        <div id="form-container" class="bg-white p-5 rounded-xl shadow border border-gray-200 xl:col-span-4 max-w-2xl mx-auto w-full max-h-none md:max-h-[85vh] overflow-y-auto pb-24 md:pb-5 transition-all duration-300">
          ...
          <form id="form-cred-txn-entry" class="space-y-4 text-xs">
            ...
            <div><label class="block font-bold text-gray-600 mb-1" data-i18n="field.categoryClassification">Category Classification</label><select id="cred-txn-category" required class="w-full border rounded p-2 bg-white text-sm outline-none focus:ring-2 focus:ring-orange-500"><option value="Received" data-i18n="field.receivedAmountCashIn">Received (Increases Due)</option><option value="Return" data-i18n="field.returnAmountCashOut">Return (Decreases Due)</option></select></div>
      ...
      <div class="grid grid-cols-1 xl:grid-cols-4 gap-6">
        <div id="form-container" class="bg-white p-5 rounded-xl shadow border border-gray-200 xl:col-span-4 max-w-2xl mx-auto w-full max-h-none md:max-h-[85vh] overflow-y-auto pb-24 md:pb-5 transition-all duration-300">
          ...
          <form id="form-inc-txn-entry" class="space-y-4 text-xs">
            ...
            <div><label class="block font-bold text-gray-600 mb-1" data-i18n="field.categoryClassification">Category Classification</label><select id="inc-txn-category" required class="w-full border rounded p-2 bg-white text-sm outline-none focus:ring-2 focus:ring-blue-500"><option value="Receivable" data-i18n="field.receivableAmountBilled">Receivable (Increases Due)</option><option value="Received" data-i18n="field.actuallyReceivedCashIn">Received (Decreases Due)</option></select></div>
      ...
      <div class="grid grid-cols-1 xl:grid-cols-4 gap-6">
        <div id="form-container" class="bg-white p-5 rounded-xl shadow border border-gray-200 xl:col-span-4 max-w-2xl mx-auto w-full max-h-none md:max-h-[85vh] overflow-y-auto pb-24 md:pb-5 transition-all duration-300">
          ...
          <form id="form-cap-txn-entry" class="space-y-4 text-xs">
            ...
            <div><label class="block font-bold text-gray-600 mb-1" data-i18n="field.categoryClassification">Category Classification</label><select id="cap-txn-category" required class="w-full border rounded p-2 bg-white text-sm outline-none focus:ring-2 focus:ring-violet-500"><option value="Capital In" data-i18n="field.capitalInAmount">Capital In (Increases Due)</option><option value="Capital Out" data-i18n="field.capitalOutAmount">Capital Out (Decreases Due)</option></select></div>
